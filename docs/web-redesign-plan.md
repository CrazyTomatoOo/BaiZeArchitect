...
- 顶部:当前 workspace 切换器(切非默认 → 防歧义三连:switcher 变色 + amber banner + `?workspace=` URL 深链)
- 中部:三区分组 ——
  - 工作:需求 · 资产库(新) · 总览
  - 治理:待决策(有待审批时 sidebar 显琥珀 chip `n`)
  - 管理:工作区 · 系统
- 底部:常驻系统状态区(`● ws 已连接` + `▶ n run 进行中 · 当前阶段`)

**作用域**:workspace 单作用域,全站页面随之过滤。需求是列表项,不当全局作用域。

**落地页**:localStorage 记住上次页(键 `baize.ui.v1.lastPage`);首次访问 —— 有 workspace → 需求页,无 → 工作区页。
