...
- [资产库 UI 落点与引用展示](tickets/R04-asset-library-ui-surface.md) — 资产库加「参与者库」tab（随 R06 改名「干系人库」）；actor→stakeholder 结构化表单；三态引用渲染（resolvedActors 随 R07 改为 resolvedGraph）；空态沿用+删除冲突提示。UI 框架保留，文案/数据源随 R06-R09 更新。
...
- [导入导出与 API 契约适配关系表层](tickets/R09-import-export-api-relations.md) — 导出单段含边表 `{assets, relations:[{fromTitle,fromKind,toTitle,toKind,type}]}`；导入 title 映射重建边（先建资产→按 title+kind 反查 assetId 建边）；PATCH 仅 stakeholder（边表不刷新 enrich 动态读最新 title）；禁删双向扫（from OR to 非空→409 asset_referenced+refs）；校验迁移到 relations 写入（server 层 create/import 校验 toAssetId 存在性+白名单+自环+同 ws，聚合 400 invalid_relations）；POST /api/assets body 加可选 relations:[{toAssetId,type}]，POST /api/assets/import body 加可选 relations:[{fromTitle,fromKind,toTitle,toKind,type}]（皆可选，不带则只建资产）。
- [终点规格与验收资产重建](tickets/R10-spec-and-acceptance-rebuild.md) — 新结构按技术层分块（schema/runtime/api）；人类评审入口 docs/workspace设计模型资产化规格.md；八维测试矩阵（CRUD/引用校验/禁删/重映射/导出/归档/关系建模/迁移）；六项评审清单（+关系类型白名单完整性+归档去重幂等性）；旧 actor-asset-spec-v1.json + docs/资产库用户角色定义规格.md 已删除；终点资产 stakeholder-asset-spec-v1.json + docs/workspace设计模型资产化规格.md 已就位。
...
- 资产规模治理：资产库较大时的列表分页/检索/去重；首版资产库无分页，不预铺。
...
- 全 kind 统一更新 API；PATCH 仅对 kind=stakeholder 开放，scenario/usecase/function 维持 create/delete。