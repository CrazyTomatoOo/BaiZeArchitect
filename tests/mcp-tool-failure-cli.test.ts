...
test("MCP tool failure produces a deterministic error path", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      mcpServers: {
        "baize-analysis": {
          command: "node",
          args: ["--import", "tsx", "tests/mcp-failure-server.ts"],
        },
      },
    }),
  );
  ...
  try {
    ...
    assert.ok(eventTypes.includes("mcp_server_started"));
    assert.ok(eventTypes.includes("mcp_tool_call"));
    ...
    assert.equal(
      (toolResult.payload as { isError?: boolean }).isError,
      true,
    );
  } finally {
    await pool.end();
