...
> 执行顺序：Wave 1 全部并行 → Wave 2 并行 → Wave 3 并行 → Wave 4 → Final Verification Wave。
> 注意（与原始计划假设的差异）：仓库非 git repo（无 worktree、无 git tag 回滚，直接原地工作）；`internal/agents/registry.go` 不存在（T4 新建）；现有认证为 Bearer/team-token 而非字面 X-Team-Token（T2 迁移到 JWT）；无 CONTEXT.md（T8 术语表以本计划 §2 领域模型为准）。

### Wave 1 — 基础设施（全部并行）

- [x] 1. (WP1) 前端工程初始化
- [x] 2. (WP2-backend) GitHub OAuth + JWT
- [x] 3. (WP3-backend) Workbench API + SSE
- [x] 4. (WP5-backend) Agent 角色体系

### Wave 2 — SPA 主体（并行；T5 依赖 T1+T2，T6 依赖 T1+T3）

- [x] 5. (WP2-frontend) 登录与认证流
- [x] 6. (WP3-frontend) Workbench SPA 核心

### Wave 3 — 审批与国际化（并行；均依赖 T6）

- [x] 7. (WP4) 审批流程
- [x] 8. (WP6) 国际化落地

### Wave 4 — 迁移与验证（T9 依赖 T5-T8；T10 依赖全部）
...
- [ ] F1. 目标/约束验证：逐条核对 §1 的 16 项设计决策是否全部落地
- [ ] F2. 代码质量审查：前后端变更的规范一致性、无 stub/TODO/占位残留
- [ ] F3. 安全审查：OAuth 流程、JWT 存储与过期、httpOnly cookie、SSE 认证、reason 必填校验
- [ ] F4. 验收标准实测：§6 全部 9 条验收标准逐项真实验证（非 dry-run），附证据
