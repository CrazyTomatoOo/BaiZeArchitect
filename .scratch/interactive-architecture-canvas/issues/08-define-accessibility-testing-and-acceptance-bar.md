...
The release browser is **Google Chrome only**. The Command-deck canvas, controls, semantic companion list, and evidence inspector retain their keyboard and semantic behavior in Chrome; the graphical canvas is never the only representation. The synchronized companion list remains the traversal surface, the canvas exposes layer/root/count summary, and a live region announces loading, result counts, selection, focus, export completion/refusal, snapshot changes, and errors. Tab reaches controls/list/inspector, list arrows move, Enter selects, and search/focus/fit/Escape retain visible focus treatment. This release makes no Safari, Firefox, VoiceOver, NVDA, or cross-browser WCAG conformance claim.
...
- **Human verification:** each release candidate completes the full keyboard-only exploration in Google Chrome and records the result. VoiceOver/Safari and NVDA/Firefox are not release gates.
