...
// ─── 1. task_output resolves to exactly one published revision ───

test("task_output input resolves to exactly one published ancestor revision before attempt creation", async () => {
	await withDepRuntime(async ({ runtime, databasePath }) => {
		...
		assert.notEqual(designBegin.taskId, 0, "design task should be ready after analysis published");
...
function expectRevisionId(database: Database.Database, workflowId: number, kind: string): number {
	return (database
		.prepare(`select ar.id from artifact_revisions ar join artifacts a on a.id = ar.artifact_id join workflows w on w.requirement_id = a.requirement_id where w.id = ? and a.kind = ? and ar.status = 'pending' order by ar.id desc limit 1`)
...
test("architect attempting to write analysis artifact is rejected by tool ownership", async () => {
	await withDepRuntime(async ({ runtime, databasePath }) => {
		...
		assert.equal(queryPublishedRevisionCount(databasePath, workflowId, "analysis"), 1, "no new analysis revision from architect");
	});
});
