...
test("all 12 reusable asset kinds are creatable with valid content", async () => {
	await withRuntime(async ({ runtime }) => {
		...
		{ kind: "data", title: "Data", content: { schemaVersion: "artifact/data/v1", artifactKind: "data", summary: "D", sourceRefs: [{ type: "requirement_revision", revisionId: 1 }], entities: [{ entityId: "e1", name: "E", fields: [{ fieldId: "f1", name: "f", type: "string" }], primaryKey: ["f1"] }] } },
