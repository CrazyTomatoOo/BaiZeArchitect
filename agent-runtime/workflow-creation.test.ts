...
test("startup refuses an unknown newer Workflow schema migration", async () => {
	...
	runtime.close();
	const database = new Database(databasePath);
	database.prepare("insert into schema_migrations(version, name, checksum, applied_at) values (4, 'future', 'sha256:future', ?)").run("2026-08-12T10:00:00.000Z");
	database.close();
	try {
		await assert.rejects(
			openHeadlessWorkflowRuntime(runtimeOptions(databasePath)),
			/Workflow database migration 4 is newer than supported version 3/,
		);
	} finally {
		await rm(directory, { recursive: true, force: true });
	}
