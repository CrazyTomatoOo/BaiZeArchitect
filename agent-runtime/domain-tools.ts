...
function clip(value: unknown): unknown {
	...
	while (JSON.stringify(clipped).length > MAX_RESULT_CHARS && preview.length > 0) {
...
async function collectFiles(repoPath: string, requested = "."): Promise<string[]> {
	const root = resolve(repoPath);
...
function consistencyReport(context: DomainToolContext): { ok: boolean; issues: unknown[] } {
	...
	for (const artifact of context.store.listArtifacts(context.requirementId)) {
		...
		for (let index = 0; index < revisions.length; index += 1) {
			...
			if (revision.revision_no !== index + 1) {
				issues.push({ type: "revision_gap", artifactId: artifact.id, revisionId: revision.id });
			}
			if (revision.fork_from_revision_id !== null) {
				const fork = context.store.getArtifactRevision(revision.fork_from_revision_id);
				if (!fork || fork.artifact_id !== artifact.id || fork.revision_no >= revision.revision_no) {
					issues.push({ type: "invalid_fork", artifactId: artifact.id, revisionId: revision.id });
				}
			}
		}
	}
	for (const decision of context.store.listDecisions(context.requirementId)) {
		if (decision.selected_option_id !== null) {
			const option = context.store.getDecisionOption(decision.selected_option_id);
			if (!option || option.decision_id !== decision.id) {
				issues.push({ type: "invalid_decision_option", decisionId: decision.id });
			}
		}
	}
...
export function createDomainTools(context: DomainToolContext) {
	const inspectRepository = defineTool({
		...
		description: "Inspect repository files through a bounded domain view; never execute shell commands.",
		...
		execute: async (_id, params) =>
			audited(context, "inspect_repository", params, async () => {
				...
				return { root: params.path ?? ".", files, truncated: files.length >= MAX_FILES };
			}),
	});
	...
	const searchCode = defineTool({
		name: "search_code",
		label: "Search Code",
		description: "Search literal code text inside the repository with bounded results; no raw grep is exposed.",
		parameters: Type.Object({
				...
				for (const filePath of files) {
					...
					for (const [index, line] of text.split("\n").entries()) {
						if (line.toLowerCase().includes(params.query.toLowerCase())) {
							matches.push({ filePath, line: index + 1, text: line.slice(0, 500) });
							if (matches.length >= MAX_MATCHES) break;
						}
					}
				}
	...
	const getArchitecture = defineTool({
		...
		description: "Read the requirement's persisted evidence snapshot and architecture facts.",
		parameters: Type.Object({}),
		execute: async (_id, params) =>
			audited(context, "get_architecture", params, () => ({
				snapshot: parseArchitecture(context.store.getEvidenceSnapshot(context.requirementId)),
			})),
	});
	...
	const searchPriorDesigns = defineTool({
		...
		description: "Search archived design package snapshots from this workspace.",
		parameters: Type.Object({ query: Type.String({ minLength: 1 }) }),
		execute: async (_id, params) =>
			audited(context, "search_prior_designs", params, () => {
				const packages = context.store.listDesignPackages(context.workspaceId) as Array<Record<string, unknown>>;
				return packages
					.map((item) => ({ item, score: scoreText(params.query, JSON.stringify(item)) }))
	...
	const patchArtifact = defineTool({
		...
		description: "Create a new immutable artifact revision, optionally forking a specific prior revision.",
		parameters: Type.Object({
			artifactId: Type.Number(),
			content: Type.Any(),
			forkFromRevisionId: Type.Optional(Type.Number()),
			status: Type.Optional(
				Type.Union([Type.Literal("draft"), Type.Literal("pending"), Type.Literal("approved"), Type.Literal("rejected")]),
			),
		}),
		execute: async (_id, params) =>
			audited(context, "patch_artifact", params, () => {
				const artifact = context.store.getArtifact(params.artifactId);
				if (!artifact || artifact.requirement_id !== context.requirementId) throw new Error("artifact not found");
				const latest = context.store.listArtifactRevisions(params.artifactId).at(-1);
	...
	const raiseDecision = defineTool({
		...
		description: "Record a decision that requires explicit human or reviewer resolution.",
		parameters: Type.Object({
			title: Type.String({ minLength: 1 }),
			question: Type.String({ minLength: 1 }),
			severity: Type.Optional(Type.String()),
			options: Type.Optional(Type.Array(Type.Object({ title: Type.String(), description: Type.Optional(Type.String()) }))),
		}),
		execute: async (_id, params) =>
			audited(context, "raise_decision", params, () => {
				...
				const options = (params.options ?? []).map((option) =>
					context.store.addDecisionOption(decision.id, option.title, option.description ?? ""),
				);
	...
	const recordFinding = defineTool({
		...
		description: "Record a traceable design or repository finding for the current requirement.",
		parameters: Type.Object({
	...
	const runConsistencyCheck = defineTool({
		...
		description: "Check artifact revision chains, decision selections, and evidence traceability.",
		parameters: Type.Object({}),
		execute: async (_id, params) => audited(context, "run_consistency_check", params, () => consistencyReport(context)),
	});

	const requestHumanInput = defineTool({
		name: "request_human_input",
		label: "Request Human Input",
		description: "Pause design progress by recording a blocking decision for a human response.",
		parameters: Type.Object({
			question: Type.String({ minLength: 1 }),
		...
		execute: async (_id, params) =>
			audited(context, "request_human_input", params, () => {
				...
				return { status: "waiting_for_human", decisionId: decision.id, reason: params.reason ?? "" };
			}),
	});
